...
val cmdseq = CMDTree.Node("CMDS", [CMDTree.Node("SET SEQ", [CMDTree.Leaf("A VAR"), CMDTree.Node("PLUS", [CMDTree.Leaf("5 INT") , CMDTree.Leaf("6 INT")])]),
CMDTree.Node("SET SEQ", [CMDTree.Leaf("C VAR"), CMDTree.Node("TIMES", [CMDTree.Leaf("11 INT") , CMDTree.Leaf("2 INT")])]),
CMDTree.Node("WH SEQ", [CMDTree.Node("NEQ BEXP", [CMDTree.Leaf("A VAR"), CMDTree.Leaf("C VAR")]),CMDTree.Node("CMDS", [CMDTree.Node("SET SEQ", [CMDTree.Leaf("A VAR"), CMDTree.Node("PLUS", [CMDTree.Leaf("A VAR") , CMDTree.Leaf("1 INT")])])]) ])])

val cmd_array = Array.array(500,"")
val counter = 0

fun get_postfix(cmd) = let
            fun postfix ast = case ast of CMDTree.Leaf ast => ast |  CMDTree.Node ast=> helper(ast) 
            and helper(str,head::tail) = (postfix head) ^" "^ helper(str, tail)
                 |helper(str, nil) = str
            val pp = postfix cmd
            in
                pp
...
fun go_to(stk, stk2) = let val topmost = if FunStack.depth(stk) >0 then FunStack.top(stk) else "NO";
                            ...
                            in  
                                if(cond = true) then (new, old, topmost) else go_to(old, new)
                            end
...
fun edit_stack_while(stk, stk2, counter) = let val wh1 = go_to(stk, stk2);                          
...
fun edit_stack_ite(stk, stk2, counter) = let val ite1 = go_to(stk, stk2);
                                val up = update(#2(ite1), #1(ite1), counter)
                                val new = reverse_stack(#1(up), #2(up))                               
                                (*else clause*)
                                val up2 = update(#1(up), new, #3(up)) 
                                (*if clause*)
                                val new = reverse_stack(#1(up2), #2(up2))
                            in
                                (new, #3(up2))
                            end 

fun edit_stack(stk, stk2, count, counter) =  if count = 0 then reverse_stack(stk, FunStack.create) else 
                                    (if #3(go_to(stk, FunStack.create)) = "WH" then 
                                    let 
...
(*change others to x*)
                                        ...
                                        let val v_top = FunStack.top(v);
                                            ...
                                            val l1 = HashTable.lookup ht (v_top);
                                            val l2 = HashTable.lookup ht (v_top2);
                                            val a1 = if Array.sub(m,l1) = 1 then true else false;
                                            val a2 = if Array.sub(m,l2) = 1 then true else false;
...
val letsc = vmc.rules (0,FunStack.create,Array.array(500,0),c) (*handle Second => (69,FunStack.create, Array.array(100,0), FunStack.create)
                                                                     |Second => (74,FunStack.create, Array.array(100,0), FunStack.create)*)
